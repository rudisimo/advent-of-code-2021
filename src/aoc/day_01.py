...
def answer_1(input: List[str]) -> int:
    measurements = deque([], maxlen=2)

    def measure(count, measurement):
        # store the next measurement in our stack
        measurements.append(measurement)

        # no previous measurement, do not count
        if count is None:
            return 0

        # fetch the previous measurement from our stack
        previous = measurements.popleft()

        # increment count when our depth is higher
        increment = 1 if measurement > previous else 0
